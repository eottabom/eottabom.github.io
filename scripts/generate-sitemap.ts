...
const generateSitemap = () => {
    ...
    const projects = (projectData.projects ?? []) as { slug: string; date?: string }[];
    ...
    const staticPages = [
        ...
        { loc: `${BASE_URL}/projects/`, changefreq: 'weekly', priority: '0.7', lastmod: today },
    ...
    const projectUrls = projects.map((project) => {
        const lastmod = toW3CDate(project.date);
        return `
    <url>
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  ${projectUrls}
    ...
    console.log(`sitemap.xml generated (${staticPages.length} static, ${posts.length} posts, ${books.length} books, ${projects.length} projects)`);
};
