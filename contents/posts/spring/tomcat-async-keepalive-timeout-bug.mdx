...
출발점은 keep-alive 가 아니라, 다른 서버가 특정 애플리케이션을 호출할 때 간헐적으로 보이던 502 였다.

평소에는 정상 응답이 왔지만, 문제 시점의 ALB access log 를 보면 아래처럼 해석되는 패턴이 반복됐다.

* `elb_status_code` 는 `502` 였다.
* `target_status_code` 는 `-` 로 찍혔다.
* `request_processing_time`, `target_processing_time`, `response_processing_time` 값도 모두 `-1` 이었다.

AWS 는 [How do I troubleshoot HTTP 502 errors in Amazon ELB?](https://repost.aws/knowledge-center/elb-alb-troubleshoot-502-errors) 문서에서 이런 로그 패턴을,

**ALB 가 target 으로 요청을 전달하지 못한 경우에 볼 수 있는 예시**로 설명한다.
...
이 시점에는 아직 "왜 target 으로 전달되지 못했는가" 만 보였고, 실제로 어떤 쪽이 먼저 연결을 닫는지는 패킷을 보기 전까지 알 수 없었다.
