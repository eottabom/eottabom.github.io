...
date: "2024-09-22"
tags: ["Java", "Lombok"]
summary: "Points to Note When Using Lombok + Disadvantages of Lombok"
description: "Java 에서 Lombok 사용 시 유의해야 할 사항과 단점에 대해서 알아보자."
---


:::info
Lombok 은 보일러플레이트(boilerpalte) 코드를 줄이기 위해 사용되는 유용한 라이브러리다.
...
:::note
`@Data` 어노테이션은 `@Getter`, `@Setter`, `@ToString`, `@EqualsAndHashCode`, final 필드나, `@NonNull` 이 붙은 필드를 초기화 하는 생성자(`@RequiredArgsConstructor`)도 생성한다.
:::
...
`@EqualsAndHashCode.Include` 또는 `@EqualsAndHashCode.Exclude` 로 사용하는 필드를 지정할 수 있다. <br/>
JPA Entity 에 대한 `equals()` 와 `hashCode()` 구현은 신중해야 한다.

JPA Entity 는 일반적으로 가변적(mutable) 인데, 이 때 `equals()` 와 `hashCode()` 메서드는 동등성 비교에 사용된다. <br/>
...
영속화 이전에는 id 가 null 이므로, `equals()`와 `hashCode()`가 같게 동작하지만, 영속화 이후 id 가 할당되면, 서로 다른 객체로 인식 되는 문제가 있다.
...
:::note
JPA 구현체가 Entity 를 리플렉션을 통해 인스턴스화 할 때 사용되고, 기본 생성자가 있어야 JPA 가 데이터베이스에서 데이터를 읽고 해당 데이터를 Entity 객체로 변환할 수 있다.
:::
...
:::warning
<b>Lombok 을 사용할 때 유의해야하는 점</b>
✔ JPA Entity 를 사용할 때 `@EqualsAndHashCode` 와 `@Data` 를 같이 사용하지 않는다.
✔ `@ToString` 을 사용할 때는 항상 lazy 속성을 제외한다.
✔ `@Builder`, `@AllArgsConstructor` 를 사용할 때 `@NoArgsConstrcutor` 를 사용해라.
:::
...
사실 실제로도 많은 커뮤니티에서는[바로가기](https://www.reddit.com/r/java/comments/mpd0gw/is_using_project_lombok_actually_an_good_idea/) `Lombok` 사용에 대한 찬성과 반대가 많이 이루어지고 있어서 좋다 나쁘다를 명확히 설명하긴 어려울 것 같다.
반대하는 입장에서는 Lombok 을 사용할 때의 단점들 때문인데, 그 단점에 대해서 간단히 알아보자.
...
위의 코드는 `@Value` 어노테이션으로 클래스를 불변으로 만들고,
모든 필드를 `private` 과 `final` 로 만들고, 모든 필드에 대한 getter 메서드를 생성한다.
`toString()`, `equals()`, `hashCode()` 메서드를 자동으로 생성하며,
기본 생성자를 `private` 으로 만들어 직접적인 인스턴스 생성을 방지하고,
모든 필드를 포함하는 생성자를 자동으로 생성한다.

`@Builder` 어노테이션을 빌더 패턴을 구현하는 내부 클래스를 생성한다.

이 코드에서 `Lombok` 을 제거했을 때 아래와 같은 코드가 된다.
...
:::success
<b>개인적인 생각 </b>
✔ Lombok 은 반복되는 코드를 줄이는데 유용한 라이브러리는 맞다고 생각한다.
✔ 하지만, JPA Entity 와 사용할 때 고려해야하는 부분들을 보았을 땐, 오히려 코드가 복잡해질 수 있다는 단점이 있는 것 같다.
✔ 또한, Java 14 부터 도입된, `record` keyword 를 사용하면 불변 객체를 쉽게 정의할 수 있는 구조체를 만들 수 있다.
➡ (`@Data`, `@Value` 와 유사하게 불변 클래스를 만들 수 있다.)
