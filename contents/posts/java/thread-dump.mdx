...
Thread Dump는 **"스레드 문제"** 라는 확신이 없어도 일단 찍어보는 것을 좋을 수 있다.

이유는 상대적으로 수집 비용이 낮고, 원인이 보이면 빠르게 다음 행동으로 넘어갈 수 있다.

보통은 아래와 같은 상황에서 Thread Dump 를 찍어볼 수 있다.

### 1) CPU 사용률이 갑자기 치솟을 때

* 어떤 스레드가 CPU를 잡고 있는지 확인할 수 있는데, `top -H`로 OS 스레드를 찾고, nid로 Thread Dump와 매핑하면 범인이 나온다.
...
* `http-nio-*`, `executor-*` 계열 스레드가 모두 요청을 처리 중인데 큐가 쌓인다면 스레드 풀 고갈을 의심할 수 있다.
* 단순히 `WAITING` 스레드가 많다는 이유만으로는 부족하다. idle worker도 정상적으로 `WAITING` 상태일 수 있다.
* idle worker와 작업 중인 worker는 스택 최상단으로 구분할 수 있다.
  - **Idle worker**: 스택 최상단이 `ThreadPoolExecutor.getTask` → `LinkedBlockingQueue.take` → `parkNanos` 패턴
  - **작업 중인 worker**: 스택 최상단에 애플리케이션 코드(`Controller`, `Service` 등)가 보임

```bash
# 작업 중인 톰캣 스레드 수 세기 (com.example을 실제 패키지명으로 변경)
jstack <pid> | grep -A 30 "http-nio" | grep "com.example" | wc -l
```

톰캣의 기본 `max-threads`는 200 인데, 덤프에서 `http-nio-8080-exec` 스레드가 200개 모두 애플리케이션 코드에서 멈춰 있다면 <BlueText>스레드 풀 고갈</BlueText>을 강하게 의심해야 한다.

:::tip
단 한 번보다 **30초 간격으로 3~5번** 연속으로 찍는 것이 좋다. 그래야 일시적 현상과 고착된 상태를 구분할 수 있다.
:::
...
:::warning
RUNNABLE이 많다고 곧장 CPU 스파이크로 단정하면 안 된다.
JVM의 RUNNABLE은 OS의 Running 상태와 다르다. SocketInputStream.socketRead0처럼 DB 응답 대기, 외부 API 호출 대기 등 native I/O 블로킹 상태도 RUNNABLE로 표시된다.
스택 트레이스를 확인해 실제로 CPU를 소모하는 루프인지, I/O를 기다리는 중인지 구분해야 한다.
CPU 스파이크 여부는 top -H + nid 매핑으로 판단한다.
:::
...
JDK 9부터 Oracle이 공식 권장하는 방법이고, 컨테이너 환경에서도 안정적으로 동작하면서, Java 21 이상에서는 가상 스레드 전용 옵션도 지원한다.

```bash
jcmd <pid> Thread.print                     # jstack과 동일
jcmd <pid> Thread.print -l                  # java.util.concurrent 락 정보 포함 (jstack -l과 동일)
jcmd <pid> Thread.print > thread-dump-$(date +%Y%m%d%H%M%S).txt
```

`ReentrantLock`, `ReadWriteLock` 등 `java.util.concurrent` 락은 `-l` 옵션 없이는 락 보유 정보가 덤프에 나타나지 않는다.
`synchronized`만 쓰는 코드가 아니라면 `-l` 옵션을 기본으로 쓰는 게 안전하다.

### kill -3

`jstack`을 설치하기 어렵거나 컨테이너 환경에서 유용하다.
프로세스의 표준 출력(stdout)으로 덤프가 출력되며, 프로세스가 종료되지 않는다.
...
Spring Boot 애플리케이션이라면 `spring-boot-starter-actuator` 의존성을 추가하고,
HTTP 노출 대상에 `threaddump`를 포함하면 된다.
...
기본적으로 JSON 형태로 응답을 받을 수 있고, `jstack`처럼 텍스트 형태로 보고 싶다면 `Accept: text/plain`을 지정한다.
...
운영 환경에서 원격으로 수집할 때 유용하지만, 보안상 엔드포인트 노출 범위는 반드시 제한해야 한다.
내부망, 별도 management port, 인증 설정 없이 외부에 열어두면 안 된다.
...
:::tip
아래 예시 코드는 <a href="https://github.com/eottabom/let-me-code/tree/main/thread-dump">let-me-code/thread-dump</a>에 정리해두었다.
각 예제는 일부러 종료되지 않거나 일정 시간 동안 CPU를 점유하도록 만들었고, 실행 방법은 <a href="https://github.com/eottabom/let-me-code/blob/main/thread-dump/README.md">모듈 README</a>에서 확인할 수 있다.
:::
...
`Found N deadlock.` 문구가 보이면 데드락이다.
락 주소(`0x...`)를 따라가면 어떤 스레드가 무엇을 보유하고 있는지 정확히 추적할 수 있다.
...
같은 락 주소(`0x000000076b3a1d40`)를 기다리는 스레드가 여러 개라면, 해당 주소를 `locked`로 보유한 스레드 찾야하는데, 바로 그 스레드가 병목이다.

```bash
# 특정 락을 기다리는 스레드 수
grep 'waiting to lock <0x000000076b3a1d40>' dump.txt | wc -l

# 그 락을 보유한 스레드 찾기
grep -B 30 'locked <0x000000076b3a1d40>' dump.txt | grep '^"'
```

:::tip
ReentrantLock, ReadWriteLock 등 java.util.concurrent 락은 위 패턴으로 잡히지 않는다.
이 경우 jcmd &lt;pid&gt; Thread.print -l로 수집해야 락 보유 정보가 덤프에 포함된다.
:::
...
CpuSpinExample을 실행하면 CPU-Spinner 스레드가 RUNNABLE 상태로 CPU를 점유한다.
`top -H`에서 가장 높은 TID가 이 스레드다.
...
## Virtual Threads (가상 스레드) 환경에서의 주의사항

Java 21 LTS에서 가상 스레드가 정식 릴리즈됐는데, `jstack`과 `jcmd Thread.print`는 carrier 스레드만 출력한다.
unmounted 상태의 가상 스레드는 덤프에 나타나지 않는다.

가상 스레드를 사용하는 환경이라면 아래 명령으로 JSON 형식의 전체 덤프를 수집해야 한다.

```bash
jcmd <pid> Thread.dump_to_file -format=json /tmp/thread-dump.json
```

이 옵션은 JDK 21 이상에서만 지원된다.
`spring.threads.virtual.enabled=true`로 가상 스레드를 활성화한 Spring Boot 3.2+ 환경이라면 이 방식이 필수다.
...
✔ 수집은 jcmd &lt;pid&gt; Thread.print -l을 기본으로, 가상 스레드 환경은 Thread.dump_to_file -format=json 사용 <br />
✔ RUNNABLE이 많아도 CPU 스파이크로 단정 금지 — 스택 트레이스로 실제 CPU 작업인지 I/O 대기인지 구분 <br />
✔ 데드락: 덤프 하단 Found N deadlock 확인 <br />
✔ BLOCKED 병목: 같은 락 주소를 기다리는 스레드 수 확인, 그 락을 보유한 스레드가 원인. java.util.concurrent 락은 -l 옵션 필수 <br />
✔ CPU 스파이크: top -H로 TID를 16진수 변환 후 nid로 매핑 <br />
✔ 스레드 풀 고갈: 스택 최상단으로 idle/작업 중 구분, 톰캣 http-nio 스레드 200개가 꽉 찼는지 확인 <br />
...
* jcmd man page - <a href="https://docs.oracle.com/en/java/javase/21/docs/specs/man/jcmd.html">Oracle - jcmd</a>
