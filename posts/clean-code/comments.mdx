...
date: "2024-05-30"
tags: ["Clean Code"]
summary: "Clean Code - About Comment Guide"
description: "Clean Code 관점에서의 Comment(주석) 남기는 방법에 대한 가이드"
---
...
<blockquote className="custom-quote">
    ...
    <footer>by Brian W. Kernighan and P. J. Plaugher</footer>
</blockquote>

코드에 주석을 추가하는 일반적인 이유는 <BlueText>코드 품질이 나쁘기 때문이다.</BlueText>
...
- 특정 결과에 대해 다른 개발자에게 경고하는 주석은 유용하다. 하지만, 문제가 있는 경우에는 해결하는 것이 가장 좋다.
...
- 아래 주석은 base64 를 인코딩하고 디코딩 할 수 있는지 테스트 하기 위해 설계된 모듈에서 추출한 주석인데, RFC 번호 이외에는 쓸모가 없다.
...
<blockquote className="custom-quote">
    <p>나쁜 코드에 주석을 달지 말고 다시 작성하라</p>
    <footer>- [The Elements of Programming Style](https://en.wikipedia.org/wiki/The_Elements_of_Programming_Style)</footer>
</blockquote>
...
- 온라인에서 찾은 코드를 사용할 때가 있는데, 소스에 대한 참조를 포함하면 다른 개발자가 아래와 같은 전체 맥락을 얻을 수 있다.
...
주석이 잘못된 코드를 변명하거나 수정하지 않는다는 점을 알 수 있다. <br />
