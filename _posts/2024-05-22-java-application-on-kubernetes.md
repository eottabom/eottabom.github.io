...
1) request / limit memory, cpu 설정하자.  <br>
2) -xss(ThreadStackSize) 는 default 값이 1M 이므로 굳이 설정하지 말자. <br>  
3) xms / -xmx 을 동일한 값으로 설정하자.  <br>
(단, 최대 힙 크기를 고정한다면 request / limit memory 의 60 ~ 80% 으로 맞추자.) <br>  
4) QoS Guaranteed 인 Pod 를 생성하자.  <br>
5) livenessProbe, readnessProbe 를 설정하자. <br>  
6) ActiveProcessorCount 플래그를 설정하는 것이 좋다.   <br>
